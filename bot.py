...
def check_key_expiration(expiration_time):
    try:
        ...
        if expiration_time == "9999-12-31T23:59:59Z":
            return True
        exp_time = datetime.fromisoformat(expiration_time.replace("Z", "+00:00"))
        return datetime.now(timezone.utc) < exp_time
...
def format_progress(checked, total=None, speed=0, found=0, target=None):
    # Speed ကို အများဆုံး 2500 ထက် မကျော်စေရန် ကန့်သတ်ထားသည်
    capped_speed = min(speed, 2500.0)
    lines = [
        "📋 Status: Running",
        f"⚡ Speed: {capped_speed:,.0f}/min",
        f"🔍 Checked: {checked:,}",
        f"💎 Found: {found}",
    ]
    if target:
