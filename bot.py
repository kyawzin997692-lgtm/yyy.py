...
# ==================== CONFIGURATION ====================
BOT_TOKEN = "8905601538:AAGgui7r7ruAoA8wD6aP3Jdg6fvlCHVxxUo"
GITHUB_TOKEN = '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'
REPO_OWNER = "kyawzin997692-lgtm"
REPO_NAME = "Kyawzinwin789893"

ADMINS = [
    "6779617599"
]

ADMIN_USERNAME = "@kyawzinwin"
...
def get_main_keyboard(user_id):
    ...
    if is_admin(user_id):
        keyboard.add(InlineKeyboardButton("⚡ Admin Panel", callback_data="cmd_admin"))
    return keyboard
...
✅ PAID USER - Unlimited Access (Admin)"""
...
async def callback_handler(call):
    ...
    if call.data == "cmd_admin":
        if not is_admin(chat_id):
            await bot.answer_callback_query(call.id, "ခွင့်ပြုချက်မရှိပါ!", show_alert=True)
            return
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("📋 Key စာရင်းကြည့်ရန်", callback_data="cmd_listkeys"),
            InlineKeyboardButton("📊 ဘော့အခြေအနေ", callback_data="cmd_status"),
            InlineKeyboardButton("🛑 စကန်ရပ်ရန် (/stop)", callback_data="menu_stop"),
            InlineKeyboardButton("🔙 Back", callback_data="menu_back")
        )
        await bot.edit_message_text("👑 **Admin Control Center**\nလိုချင်သော လုပ်ဆောင်ချက်ကို နှိပ်ပါ -", chat_id=chat_id, message_id=call.message.message_id, reply_markup=markup, parse_mode="Markdown")
        await bot.answer_callback_query(call.id)
        return

    if call.data == "cmd_listkeys":
        if not is_admin(chat_id):
            await bot.answer_callback_query(call.id, "No Permission", show_alert=True)
            return
        fake_msg = call.message
        fake_msg.chat.id = chat_id
        await listkeys(fake_msg)
        await bot.answer_callback_query(call.id)
        return
    ...
    if call.data == "cmd_status":
        ...
        fake_msg = call.message
        fake_msg.chat.id = chat_id
        await status(fake_msg)
        await bot.answer_callback_query(call.id)
        return

    if call.data == "menu_back":
        if is_admin(user_id) or user_id in paid_users or user_id in approve:
...
PAID USER ဖြစ်ရန် အောက်ပါ Menu မှ PAID USER ကိုနှိပ်ပါ။"""
    ...
    if call.data == "menu_free_trial":
        if not is_admin(user_id) and user_id not in paid_users and user_id not in approve:
            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
        ...
        try:
            ...
            if portal_url != last_url and portal_url != 'Unknown':
                ...
                for admin_id in ADMINS:
                    try:
                        await bot.send_message(int(admin_id), admin_msg, parse_mode="Markdown")
                    except:
                        pass
...
Key ရရှိပြီးပါက PAID USER ဖြစ်ရန် နှိပ်ပါ"""
    ...
    if call.data == "menu_enter_userid":
        ...
        if is_admin(user_id) or user_id in auth_list:
            valid = True
...
async def handle_key(message):
    ...
    if is_admin(user_id):
        approve[message.chat.id] = True
        paid_users[user_id] = True
        await bot.reply_to(message, "👑 Admin Key အတည်ပြုပြီးပါပြီ။")
        return
    ...
    try:
        while True:
            ...
            if time.monotonic() - last_key_check >= 600:
                auth_list, _ = await get_file_content("auth_list.json")
                if not is_admin(chat_id) and str(chat_id) not in auth_list and str(chat_id) not in paid_users:
...
async def start_polling():
    ...
    while True:
        try:
            # skip_pending=True ထည့်သွင်းပေးထားပါသည် (Webhook Error 409 အတွက်)
            await bot.infinity_polling(timeout=20, request_timeout=20, skip_pending=True)
            return
